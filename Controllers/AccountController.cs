    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                user.Email = model.Email;//new
               user.EmailConfirmed = false; //new
                
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    System.Net.Mail.MailMessage m = new System.Net.Mail.MailMessage(
                       new System.Net.Mail.MailAddress("kokobosh@gmail.com", "Web Registration"),
                       new System.Net.Mail.MailAddress(user.Email));
                    m.Subject = "Email confirmation";
 m.Body = string.Format("Dear {0}<BR/>Thank you for your registration, please click on the below link to "+
                    "comlete your registration: <a href=\"{1}\" title=\"User Email Confirm\">{1}</a>",
                    user.UserName, Url.Action("ConfirmEmail", "Account", new { Token = user.Id, Email = user.Email }, Request.Url.Scheme));
                    m.IsBodyHtml = true;

                    Debug.WriteLine("mail body"+m.Body);

                    System.Net.Mail.SmtpClient smtp = new System.Net.Mail.SmtpClient("smtp.gmail.com");
                    smtp.Credentials = new System.Net.NetworkCredential("kokobosh@gmail.com", "Gocroc1234");
                    smtp.EnableSsl = true;
                    smtp.Port = 587;
                    smtp.Send(m);
        ...
        public async Task<ActionResult> ConfirmEmail(string token ,string email)
        {
          ...
          /*  if (userId == null || email == null)
            ...
            var result = await UserManager.ConfirmEmailAsync(userId, email);
